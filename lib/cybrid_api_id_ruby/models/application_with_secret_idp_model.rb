  ...
  class ApplicationWithSecretIdpModel
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiId::ApplicationWithSecretIdpModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiId::ApplicationWithSecretIdpModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @ip_allowlist.nil?
        invalid_properties.push('invalid value for "ip_allowlist", ip_allowlist cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @ip_allowlist.nil?
    ...
    def hash
      [name, client_id, scopes, ip_allowlist, created_at, updated_at, secret].hash
    end
