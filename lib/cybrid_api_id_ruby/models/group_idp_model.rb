  ...
  class GroupIdpModel
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiId::GroupIdpModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiId::GroupIdpModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'member_guids')
        if (value = attributes[:'member_guids']).is_a?(Array)
          self.member_guids = value
        end
      end
    ...
    def list_invalid_properties
      ...
      if @member_guids.nil?
        invalid_properties.push('invalid value for "member_guids", member_guids cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @member_guids.nil?
    ...
    def hash
      [guid, name, type, member_guids, created_at, updated_at].hash
    end
